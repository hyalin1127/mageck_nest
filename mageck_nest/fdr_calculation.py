...
def fdr_pounds_continuous(pvalues):
    ...
    """
    idx = sorted(list(range(len(pvalues))), key=pvalues.__getitem__)
...
def pFDR(pvalues, method='fdr'):
  all_pvalues=pvalues
  all_pvalues_n=len(pvalues)
  nan_index=[nan_index for nan_index,i in enumerate(pvalues) if math.isnan(i)==True]
  nan_index.sort()
  break_points=[0]+[(i-nan_index.index(i)) for i in nan_index]+[all_pvalues_n]
  pvalues=[i for i in pvalues if math.isnan(i)==False]
  """
  calculate the FDR of p values using false discovery rate
  ...
  fragmented_new_pvalues=[new_pvalues[break_points[i]:break_points[i+1]] for i in range(len(break_points)-1)]
  final_new_pvalues=fragmented_new_pvalues[0]
  for i in range(len(fragmented_new_pvalues)-1):
      final_new_pvalues+=[1]
      final_new_pvalues+=fragmented_new_pvalues[i+1]

  return final_new_pvalues
